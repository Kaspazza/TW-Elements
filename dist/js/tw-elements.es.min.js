...
const zo = new NC(), ki = {
  ...
  console.log("document:", t.document);
  const i = Object.keys(ki).map((n) => {
    if (!!e.querySelector(ki[n].selector)) {
      const r = s[ki[n].name];
      return !r && !zo.isInited(n) && t.checkOtherImports && console.warn(
        `Please import ${ki[n].name} from "tw-elements" package and add it to a object parameter inside "initTE" function`
      ), r;
    }
  });
  PC(i, t);
